...
class bcistream(object):
    ...
    def __init__(self, filename, ind=-1):
        ...
        for k,v in list(self.statedefs.items()):
            ...
            startmask = int( 255 & (255 << startbit) )
            endmask   = int( 255 & (255 >> extrabits) )
            div       = int(1 << startbit);
            v['slice'] = slice(startbyte, int( startbyte+nbytes ) )
    ...
    def read(self, nsamp=1, apply_gains=True):
        ...
        raw = self.file.read(int(self.bytesperframe*nsamp))
